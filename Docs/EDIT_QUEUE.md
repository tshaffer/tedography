...
- **Clear Edit Folder** — deletes all files from `TEDOGRAPHY_EDIT_PATH` (two-step confirmation required); use this after a successful import to clean up. Safe to use freely: imported `_edited` files are moved to the source photo's folder during import, so they are no longer in the edit folder by the time you clear it.
