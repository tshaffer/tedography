...
In the Edit Queue dialog's list, a photo's filename normally links in blue. If it turns **amber** instead, there's an `_edited` file already sitting in the folder for it that Import can't currently see — its manifest link is missing (most often because it hasn't been exported since a manifest overwrite predating the fix above, or it was never exported at all after producing the edit). Hover the filename to see which file(s) are stuck; checking the box and clicking **Export** again restores the link.
